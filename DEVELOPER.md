...
│       ├── installer/       # AWS config processing and profile generation
...
#### Core Libraries
- **awscli package** - 84.7% test coverage, handles AWS CLI config parsing with support for SSO, IAM, and AssumeRole profiles
- **installer package** - 93.0% test coverage, processes AWS configurations and generates profiles
- **logging package** - 100% test coverage, provides structured logging with debug support
- **core package** - 96.7% test coverage, manages settings and application state

## Testing

**📖 Complete Testing Guide**: See [`docs/TESTING.md`](docs/TESTING.md) for comprehensive testing standards, patterns, and best practices.
...
```go
import "aws-profile-manager/src/lib/test"
...
go test ./src/lib/core/... -v
go test ./src/lib/gui/... -v
```
...
go run src/main.go --help    # Test CLI directly
npm run make:test           # Run tests
npm run make:fmt            # Format code

# GUI development and testing
npm run run:gui             # Build and launch GUI
go run src/main.go gui      # Launch GUI directly
```
...
# Or set temporarily
AWS_PROFILE_MANAGER_DEBUG=1 go run src/main.go gui
```
...
find src/lib -name "*.go" -not -name "*_test.go" | while read f; do
