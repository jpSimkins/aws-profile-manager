...
// backup package (internal/backup/api.go)
result, err := backup.ExportProfiles(options)
result, err := backup.ImportProfiles(options)

// sync package (internal/sync/api.go)
result, err := sync.FetchConfig(settings, options)
result, err := sync.GetSyncStatus(settings, options)
```
...
**All business logic with external processes or long-running operations MUST use the task package.**

```go
// ✅ CORRECT: Business logic accepts task.Reporter
func DoWork(ctx context.Context, opts Options, reporter task.Reporter) (*Result, error) {
    reporter.ReportStatus("Starting work...")
    ...
    if err != nil {
        return nil, fmt.Errorf("work failed: %w", err)
    }
    
    reporter.ReportStatus("Work complete")
    return &Result{Data: result.Output}, nil
}

// ✅ CORRECT: CLI passes task.CliReporter
func runMyCommand(cmd *cobra.Command, args []string) error {
    reporter := task.CliReporter{}  // Prints to console automatically
    result, err := mypackage.DoWork(ctx, opts, reporter)
    if err != nil {
...
// ✅ CORRECT: GUI passes reporter and handles in goroutine
func doWork(window fyne.Window, progressDialog *components.ProgressDialog) {
    go func() {
        reporter := task.NoOpReporter{}  // Using own progress dialog
        result, err := mypackage.DoWork(ctx, opts, reporter)
        
        fyne.Do(func() {  // Update UI on main thread
            progressDialog.Hide()
            if err != nil {
                dialog.ShowError(err, window)  // Show error dialog
                return
            }
            showSuccess(result)
        })
...
// ❌ WRONG: Business logic doesn't accept reporter
func DoWork(opts Options) error {
    cmd := exec.Command("aws", "s3", "cp", "...")
    output, err := cmd.Output()  // No progress reporting!
    return err
}

// ❌ WRONG: CLI manually tracking progress
func runMyCommand() error {
    fmt.Println("Starting...")  // Use task.CliReporter instead!
    result := pkg.DoWork()
    fmt.Println("Done")
}
```
...
**Requirements**:
- **Business Logic**: Accept `task.Reporter`, use `task.SubprocessTask`, call `reporter.ReportStatus()`
- **CLI**: Pass `task.CliReporter{}` (auto-prints to console)
- **GUI**: Pass `task.NoOpReporter{}` or `task.ChannelReporter`, handle errors with `dialog.ShowError()`
- **Benefits**: Same code serves CLI/GUI, progress works automatically, context cancellation supported
...
│   ├── installer/             # Profile installation
...
└─────────────────────────────────────┘
              ↓ Call API
┌─────────────────────────────────────┐
│   Package APIs (*/api.go)           │  Orchestrate → Process → Return
│   - awscli.ListProfiles()           │
│   - installer.InstallProfiles()     │
│   - sync.FetchConfig()              │
└─────────────────────────────────────┘
```
...
```go
result, err := sync.FetchConfig(settings, options)
status, err := sync.GetSyncStatus(settings, options)
instructions, err := sync.GetSetupInstructions(settings)
```

**Files**:
- `api.go` - High-level functions
- `manager.go` - Orchestration and caching
- `router.go` - Strategy selection
- `http_syncer.go` - HTTP/HTTPS fetching
- `s3_syncer.go` - S3 fetching
- `local_syncer.go` - Local file system
- `types.go` - Data structures
